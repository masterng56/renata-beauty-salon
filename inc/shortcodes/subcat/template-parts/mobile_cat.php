<div class="amts_mobile_accordion">
	<?php foreach ($categories as $category) : ?>
		...
		<div class="amts_mobile_accordion_item">
			...
			<?php if ($has_subcategories) : ?>
				<div class="amts_mobile_accordion_content">
					<?php foreach ($subcategories as $subcategory) : ?>
						...
						<div class="subcategory_mobile_item">
							<?php if ($has_sub_subcategories) : ?>

								<div class="subcategory_mobile_header subcategory_mobile_header_subcat">
									<span class="subcategory_mobile_header_title">
							...
							<?php if ($has_sub_subcategories) : ?>
								<div class="sub_mobile_subcategory_content">
									<?php foreach ($sub_subcategories as $sub_subcategory) : ?>
										<div class="sub_mobile_subcategory_item">
...
<style>
	.amts_mobile_accordion_item {
		margin-bottom: 10px;
	}

	.amts_mobile_accordion_header {
		cursor: pointer;
		padding: 10px;
	...
	.amts_mobile_accordion_header:hover {
		background-color: var(--bluelite);
		border: 1px solid var(--bluelite);
		transition: all 0.5s ease;
	}

	.amts_mobile_accordion_content {
		display: none;
		padding: 10px;
	...
	.subcategory_mobile_header {
		/*background-color: var(--graybody);*/
		cursor: pointer;
	}

	.subcategory_mobile_header_subcat {
		position: relative;
	}

	.subcategory_mobile_header_title{
		display: flex;
		align-items: center;
		color: var(--bluelite);
	}

	.subcategory_mobile_header_title:after {
		content: '';
		height: 10px;
		width: 10px;
		position: absolute;
	...
	.active.subcategory_mobile_header_title:after {
		transform: translate(-2px, 0) rotate(135deg);
		transition: all 0.4s linear 0s;
	}

	.sub_mobile_subcategory_content {
		/* display: none;  */
	}
</style>
<script>
	jQuery(document).ready(function($) {
		$('.amts_mobile_accordion_header, .subcategory_mobile_header').on('click', function() {
			$(this).next().slideToggle();
			$(this).siblings().not($(this).next()).slideUp();
		});
		$('.subcategory_mobile_header_title').on('click', function() {
